...
namespace MQSimEngine
{
	...
	{
		//_ObjectList.end() = unordered_map의 마지막 공백 즉 NULL이다. 
	...
	{
		sim_time_type prev_time = 0;// Added code for using FLIN Algorithm. If it isn't exsited, FLIN Algorithm makes infinity loop. By Seong Jihwan
		started = true;
		...
		while (true) {
			if (_EventList->Count == 0 || stop) {	
				if (stop == true)
				{
					std::cout << "stop is ture " << std::endl;

				}
				else
				{
					std::cout << "_EventList->Count is 0 " << std::endl;
				}
				break;
			}
	...
	int Engine::Get_sim_event_count()
	{
