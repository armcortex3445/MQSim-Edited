	...
	case NVM::NVM_Type::FLASH:
	{
		...
		case SSD_Components::Flash_Scheduling_Type::FLIN:
				{
					...
					for (auto& flow : (*io_flows))
					{
							stream_ids_per_priority_class[flow->Priority_Class][idx[flow->Priority_Class]] = j;
							idx[flow->Priority_Class]++;
							j++;
					}
					...
					tsu = new SSD_Components::TSU_FLIN(ftl->ID() + ".TSU", ftl, static_cast<SSD_Components::NVM_PHY_ONFI_NVDDR2*>(device->PHY),
						...
						10000000, 327680, 102400, 4, (unsigned int)io_flows->size(), stream_count_per_priority_class, stream_ids_per_priority_class,
						0.6,
						...
						erase_suspension, program_suspension);
					for (int i = 0; i < 4; i++)
						delete[] stream_ids_per_priority_class[i];
					delete[] stream_count_per_priority_class;

					break;
				}
		case SSD_Components::Flash_Scheduling_Type::CIF:
			{
					unsigned int * stream_count_per_priority_class = new unsigned int[4];
					for (int i = 0; i < 4; i++)
						stream_count_per_priority_class[i] = 0;
					...
					tsu = new SSD_Components::TSU_CIF(ftl->ID() + ".TSU", ftl, static_cast<SSD_Components::NVM_PHY_ONFI_NVDDR2*>(device->PHY),
						parameters->Flash_Channel_Count, parameters->Chip_No_Per_Channel,
						parameters->Flash_Parameters.Die_No_Per_Chip, parameters->Flash_Parameters.Plane_No_Per_Die, parameters->Flash_Parameters.Page_Capacity, 4,
						(unsigned int)io_flows->size(),   stream_count_per_priority_class, stream_ids_per_priority_class,
						parameters->Preferred_suspend_write_time_for_read, parameters->Preferred_suspend_erase_time_for_read, parameters->Preferred_suspend_erase_time_for_write,
						erase_suspension, program_suspension);
		...
		for (unsigned int i = 0; i < io_flows->size(); i++)
		{
			...
			case HostInterface_Types::SATA:
			{
				...
				/* Structure of flow_channel_id_assignments.     
				*								   flash_channel_ID_type
				*  vector [io_flow 0]  :  channel_ids   = [0] - [1] - [2] - ..[Flash_Channel_Count]													
				*  vector [io_flow 1]  :  channel_ids   = [0] - [1] - [2] - ..[Flash_Channel_Count]		 
				By Soeng Jihwan   
				*/
				...
				/* Structure of flow_chip_id_assignments    
				*								   flash_chip_ID_type
				*  vector [io_flow 0]  :  chip_ids   = [0] - [1] - [2] - ..[Chip_No_Per_Channel]													
				*  vector [io_flow 1]  :  chip_ids   = [0] - [1] - [2] - ..[Chip_No_Per_Channel]	
				* By Soeng Jihwan  
				*/
			...
			case HostInterface_Types::NVME:
			{
				...
				/* Structure of flow_channel_id_assignments       
				*								   flash_channel_ID_type
				*  vector [0번째 io_flow]  :  channel_ids   = [(*io_flows)[0]->Channel_IDs[0]] - [(*io_flows)[0]->Channel_IDs[1]] - ...													
				*  vector [1번째 io_flow]  :  channel_ids   = [(*io_flows)[1]->Channel_IDs[0]] - [(*io_flows)[1]->Channel_IDs[1]] - ...		
				* By Seong Jihwan
				*/
