																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
	...
	void IO_Flow_Base::NVMe_consume_io_request(Completion_Queue_Entry* cqe)
	{
		...
		// Added code to check overflow of simuation time. By Seong Jihwan 
		if (Simulator->Time() < request->Enqueue_time) {
			std::cout << "#####request->Enqueue_time is overflow#####" << request->Enqueue_time << " and simulator time is " << Simulator->Time() << std::endl;
		}
		sim_time_type request_delay = Simulator->Time() - request->Arrival_time;
		STAT_serviced_request_count++;
		//FLIN 문제를 해결하기 위해 추가함.	  
		STAT_serviced_request_count_short_term++;

		//statistic 변수들 갱신
		sim_time_type prev_STAT_sum_deivce_response_time = STAT_sum_device_response_time;
		STAT_sum_device_response_time += device_response_time;
		sim_time_type prev_STAT_sum_request_delay = STAT_sum_request_delay;
		STAT_sum_request_delay += request_delay;

		// Added code to check overflow of simuation time. By Seong Jihwan  
		if (prev_STAT_sum_deivce_response_time > STAT_sum_device_response_time)
		{
			std::cout << "#####device_response_time is over flow#####" << "overflowed device response time :"<<STAT_max_device_response_time << std::endl;
			

		}
		if (prev_STAT_sum_request_delay > STAT_sum_request_delay)
		{
			std::cout << "#####STAT_sum_request_delay is over flow#####" << "overflowed request delay time :" << STAT_sum_request_delay <<"and simulator time is" << Simulator->Time() << std::endl;

		}
		STAT_sum_device_response_time_quotien += device_response_time/ SIM_TIME_TO_MICROSECONDS_COEFF;
		STAT_sum_device_response_time_remainder += device_response_time % SIM_TIME_TO_MICROSECONDS_COEFF;

		STAT_sum_request_delay_qutoien +=  request_delay / SIM_TIME_TO_MICROSECONDS_COEFF;
		STAT_sum_request_delay_remainder += request_delay % SIM_TIME_TO_MICROSECONDS_COEFF;
	...
	uint32_t IO_Flow_Base::Get_device_response_time()
	{
		...
		std::cout << "flow-" << this->flow_id << " : STAT_sum_device_response_time = " << STAT_sum_device_response_time << std::endl;
		sim_time_type device_response_time = STAT_sum_device_response_time_quotien + (STAT_sum_device_response_time_remainder /SIM_TIME_TO_MICROSECONDS_COEFF);
		return (uint32_t)(device_response_time/ STAT_serviced_request_count);
	}
	...
	uint32_t IO_Flow_Base::Get_end_to_end_request_delay()
	{
		...
		sim_time_type end_to_end_request_delay = STAT_sum_request_delay_qutoien + (STAT_sum_request_delay_remainder / SIM_TIME_TO_MICROSECONDS_COEFF);
		return (uint32_t)(end_to_end_request_delay/ STAT_serviced_request_count);
	}
	...
	void IO_Flow_Base::Report_results_in_XML(std::string name_prefix, Utils::XmlWriter& xmlwriter)
	{
		...
		attr = "Serviced_Request_Count";
		val = std::to_string(Get_serviced_request_count());
		xmlwriter.Write_attribute_string(attr, val);
