...
//Simulation start
/* in trace file, line has 5 values representing IO request status.
*  1 line = > [Request_Arrival_Time] [Device_Number] [Starting_Logical_Sector_Address] [Request_Size_In_Sectors] [Type_of_Requests[0 for write, 1 for read]]
* current_trace_line is vector type data structre. It has IO request status from 1 line in trace file.
* current_trace_line = [Request_Arrival_Time] [Device_Number] [Starting_Logical_Sector_Address] [Request_Size_In_Sectors] [Type_of_Requests(0 for write, 1 for read)]
* By Seong Jihwan
*/
...
void IO_Flow_Trace_Based::Execute_simulator_event(MQSimEngine::Sim_Event *)
{
	...
	{
		std::cout << "all request is created." << std::endl;
	}
}

/* Edited Function for successfully operating Precondition setting. by SeongJihwan
* 
*/
...
{
	stats.Domain_start_LSA = start_lsa_on_device; stats.Domain_end_LSA = end_lsa_on_device;
	...
	while (std::getline(trace_file_temp, trace_line))
	{
		...
		// Edited by Seong Jihwan
		if (start_LBA>= start_lsa_on_device && start_LBA <= end_lsa_on_device)
		{
			start_LBA = start_LBA - start_lsa_on_device;
			//start_LBA += start_lsa_on_device;
		}
		else
		{
			start_LBA = start_LBA % (end_lsa_on_device - start_lsa_on_device+1);
			//start_LBA = start_lsa_on_device + start_LBA % (end_lsa_on_device - start_lsa_on_device);
		}
		LHA_type count = + LBA_count; // sector size = end_LBA - start_LBA +1 . Ok? by Seong Jihwan. 
		//
	...
	// added by Seong Jihwan
	stats.generate_aligned_addresses = 0;
	stats.alignment_value = 0;
}
} // namespace Host_Components