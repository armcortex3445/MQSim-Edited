...
namespace NVM
{
	namespace FlashMemory
	{
		...
		public:
			...
			void StartCMDXfer() //CMD transfer가 마지막에 시행된 시각을 갱신
			{
			...
			//CMD trasfer를 종료하고, target die에서 CMD를 실행한다.
			void EndCMDXfer(Flash_Command* command)//End transferring write data to the Flash chip
			{
				...
				if (this->idleDieNo != die_no)//Active Die가 계속 존재하면, 실행. CMD를 다른 Die에서도 overlap하여 사용중
			...
			//command 실행에 따른 Latency를 계산하여 반환
