...
void Input_Stream_Manager_NVMe::segment_user_request(User_Request *user_request)
{
	...
	LHA_type lsa = user_request->Start_LBA; //Logical sector addr. 1 sector is 512byte
	...
	while (handled_sectors_count < req_size)
	{
		...
		transaction_size = host_interface->sectors_per_page - (unsigned int)(lsa % host_interface->sectors_per_page); //TR의 크기 설정.
		...
		access_status_bitmap = temp << (int)(internal_lsa % host_interface->sectors_per_page); //partial write/read 
		...
		lsa = lsa + transaction_size; //이제 lsa % host_interface->sectors_per_page = 0이 된다. 
