...
namespace SSD_Components {
	...
	sim_time_type NVM_PHY_ONFI_NVDDR2::Estimate_transfer_and_memory_time(NVM_Transaction_Flash* transaction)
	{
		ONFI_Channel_NVDDR2* target_channel = channels[transaction->Address.ChannelID];
		NVM::FlashMemory::Flash_Chip* targetChip = target_channel->Chips[transaction->Address.ChipID];
		sim_time_type transfer_and_memory_time = 0;
		 
		switch(transaction->Type){
			case Transaction_Type::READ:
				transfer_and_memory_time += targetChip->Get_command_execution_latency(CMD_READ_PAGE,transaction->Address.PageID)
				                             +NVDDR2DataInTransferTime(transaction->Data_and_metadata_size_in_byte,target_channel)
												+target_channel->ReadCommandTime[1];
				break;
			case Transaction_Type::WRITE:
				transfer_and_memory_time += targetChip->Get_command_execution_latency(CMD_PROGRAM_PAGE, transaction->Address.PageID)
											+NVDDR2DataOutTransferTime(transaction->Data_and_metadata_size_in_byte, target_channel)
											+target_channel->ProgramCommandTime[1];
				break;
		}
		return transfer_and_memory_time;
	}
	...
	inline void NVM_PHY_ONFI_NVDDR2::handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip* chip, NVM::FlashMemory::Flash_Command* command)
	{
		...
		case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
		{
			...
			if (chipBKE->No_of_active_dies == 0 && chipBKE->WaitingReadTXCount != 0)
				printf("WaitingReadTXCount는 0이 아니다.");
			if (chipBKE->No_of_active_dies == 0 && chipBKE->WaitingReadTXCount == 0)
				chipBKE->Status = ChipStatus::IDLE;
