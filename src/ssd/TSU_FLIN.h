...
namespace SSD_Components
{
	...
	struct FLIN_Flow_Monitoring_Unit
	{
		...
		unsigned int Sum_read_slowdown = 0;
		unsigned int Sum_write_slowdown = 0;
		uint64_t transaction_count;
		uint64_t Sum_RD_TR_shared_turnaround_time;
		uint64_t Sum_RD_TR_alone_turnaround_time;
		uint64_t Sum_WR_TR_shared_turnaround_time;
		uint64_t Sum_WR_TR_alone_turnaround_time;
	};
	...
	*/
	...
	public:
		...
		void estimate_flow_slowdown(NVM_Transaction_Flash* serviced_TR);
	private:
		...
		sim_time_type flow_classification_epoch;					// epoch of classfication , as described in 5.1 of the FLIN paper.
		unsigned int alpha_read_for_epoch, alpha_write_for_epoch;	// threshold for classifying which Flow is High intensity or Row intenstiy  , as described in 5.1 of the FLIN paper.
		...
		/* Added member variables By Seongh Jihwan
		* These variables represents overhead of FLIN Algorithm.  
		* These variables are printed out on result xml files.  
		* 
		* reordering_success loop_count is equal or lower than reordering_loop_count. 
		*/
		uint64_t reordering_loop_count=0; // In reorder_for_fairness() , count of loop to find position for maximizeing Fairness. This variable is used for esimating overhead of FLIN Algorithm. 
		uint64_t reordering_success_loop_count = 0;// In reorder_for_fairness(), count of loop to move to position founded. This variable is used for esimating overhead of FLIN Algorithm. 


		/* Modified OR Added Functions By Seong Jihwan
		*/
		...
		sim_time_type estimate_alone_waiting_time(Flash_Transaction_Queue* queue, std::list<NVM_Transaction_Flash*>::iterator position);
		void update_head_high_TR(Flash_Transaction_Queue::iterator transaction);

		double fairness_based_on_average_slowdown(flash_channel_ID_type channel_id, flash_chip_ID_type chip_id, unsigned int priority_class, bool is_read, stream_id_type& flow_with_max_average_slowdown);
		...
		uint16_t FLIN_Sim_event_loop_count = 0;
	};
}
