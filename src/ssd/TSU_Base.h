...
{
	OUT_OF_ORDER,			// 0
	PRIORITY_OUT_OF_ORDER,	// 1
	FLIN,					// 2
	CIF
};
class FTL;
//Sim_object class에서 상속받는다.
...
protected:
	...
	// describtion of funcion is at defination of fucntion.
	bool issue_command_to_chip_FLIN(Flash_Transaction_Queue* sourceQueue, Flash_Transaction_Queue* UserQueue, unsigned int no_of_priority_classes, bool suspensionRequired, __int8 TRslotExisted);
	virtual void estimate_flow_slowdown(NVM_Transaction_Flash* serviced_TR)
	{

	}
	
	virtual void update_head_high_TR(std::list<NVM_Transaction_Flash*>::iterator transaction)
	{

	}

	virtual void update_transaction_count(NVM_Transaction_Flash* transaction)
	{

	}
	//
	
	//dded Function for using CIF Algorithm. By Seong jihwan.
	bool issue_command_to_chip_CIF(Flash_Transaction_Queue* sourceQueue, Flash_Transaction_Queue* userqueue, unsigned int no_of_priority_classes, bool suspensionRequired, __int8 TRslotExisted);
