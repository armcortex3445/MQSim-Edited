...
namespace SSD_Components
{
		...
		PlaneAllocationScheme(PlaneAllocationScheme), Channel_no(channel_no), Chip_no(chip_no), Die_no(die_no), Plane_no(plane_no)
	{
		...
		Total_translation_pages_no = MVPN_type(Total_logical_pages_no / Translation_entries_per_page+1); 
			...
			Block_no_per_plane, Page_no_per_block, SectorsPerPage, PageSizeInByte, Overprovisioning_ratio, sharing_mode, fold_large_addresses)
	{
		...
		std::cout << " total flow is  " << no_of_input_streams << std::endl;
	...
	*/
	...
	void Address_Mapping_Unit_Page_Level::allocate_page_in_plane_for_user_write(NVM_Transaction_Flash_WR* transaction, bool is_for_gc)
	{
		...
		* may decide to move a page that is just invalidated.*/
		...
		if (block_manager->Block_has_ongoing_gc_wl(transaction->Address)&&!is_lpa_locked_for_gc(transaction->Stream_id,transaction->LPA))
		{
			Set_barrier_for_accessing_lpa(transaction->Stream_id, transaction->LPA);
		}
	}
	...
	{   
		...
		if (block_manager->Block_has_ongoing_gc_wl(read_address) && !is_lpa_locked_for_gc(stream_id, lpa))
		{
			Set_barrier_for_accessing_lpa(stream_id, lpa);
		}
	...
	//Edited Function for successfully operate MQ Sim. by Seong Jihwan.
	void Address_Mapping_Unit_Page_Level::generate_flash_writeback_request_for_mapping_data(const stream_id_type stream_id, const LPA_type lpn)
	{
		...
		if (is_mvpn_locked_for_gc(stream_id, mvpn)) {
			...
			domains[stream_id]->DepartingMappingEntries.insert(get_MVPN(lpn, stream_id));
			/* Becuase of This code, Bug occured. So edited. 
	...
	// added function for resolving precondition setting problem. By Seong Jihwan.
	...
	//Edited Function for solving race condtion about setting barrier, during Garbage Collecion. by Seong Jihwan
		...
		for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
			if (block_manager->Is_page_valid(block, pageID)) {
				...
				if (block->Holds_mapping_data) {
					...
					if (mpvn==(MVPN_type)NO_LPA)
					{
						continue;
					}
					else if (mpvn>domains[block->Stream_id]->Total_translation_pages_no)
					{
					...
					if (domains[block->Stream_id]->GlobalTranslationDirectory[mpvn].MPPN != Convert_address_to_ppa(addr)) {
						PRINT_ERROR("Inconsistency in the global translation directory when locking an MPVN!")
							...
							//previou location : Set_barrier_for_accessing_mvpn(block->Stream_id, mpvn);
					}
					Set_barrier_for_accessing_mvpn(block->Stream_id, mpvn);// Code line moves to here
				} else {
					...
					// Added code for debugging about race condition.  By Seong Jihwan.
					if (lpa == NO_LPA)
					{
						//std::cout << "lpa == NO_LPA (streamid lpa,page write index )" << block->Stream_id << " ," << lpa << " ," << block->Current_page_write_index << std::endl;
					...
					else if (lpa > domains[block->Stream_id]->Total_logical_pages_no)
					{
					...
					if (ppa != Convert_address_to_ppa(addr)) {
						std::cout << "(LPA, PPA in GMT, PPA translated by addr, value of ppa variable, stream_id ) = "
	...
	inline void Address_Mapping_Unit_Page_Level::Remove_barrier_for_accessing_lpa(stream_id_type stream_id, LPA_type lpa)
	{
		...
		if (itr == domains[stream_id]->Locked_LPAs.end()) {
			...
			if (ppa == NO_PPA)
			{
				std::cout << "ppa is not allocated ." << std::endl;

			}
			else
			{
				std::cout << "ppa is allocated = " <<ppa<< std::endl;
			...
			if (lpa > domains[stream_id]->Total_logical_pages_no)
			{
	...
	inline void Address_Mapping_Unit_Page_Level::Remove_barrier_for_accessing_mvpn(stream_id_type stream_id, MVPN_type mvpn)
	{
		...
		if (itr == domains[stream_id]->Locked_MVPNs.end()) {
			...
			{
				std::cout << "ppa is allocated . ppa = " << mppn << std::endl;
