...
namespace SSD_Components
{
	...
	void FTL::Perform_precondition(std::vector<Utils::Workload_Statistics*> workload_stats)
	{
		...
		for (auto &stat : workload_stats)
		{
			...
			if (stat->Type == Utils::Workload_Type::TRACE_BASED)
			{
			...
			std::multimap<int, LPA_type, std::greater<int>> trace_lpas_sorted_histogram;//only used for trace workloads (access 횟수, LPA)
			...
			std::cout << "precondtion step 1 " << std::endl;
			if (stat->Type == Utils::Workload_Type::SYNTHETIC)
			{
				bool is_read = false;
				unsigned int size = 0;
				...
				std::cout << " Goal LPA count  : " << no_of_logical_pages_in_steadystate << std::endl; //Added Code for debugging by Seong Jihwan.
				...
				while (lpa_set_for_preconditioning.size() < no_of_logical_pages_in_steadystate) {
					//수정
					start_LHA = random_address_generator->Uniform_ulong(0, stat->Domain_end_LSA- stat->Domain_start_LSA );
					unsigned int handled_sectors_count = 0;
					LHA_type lsa = start_LHA;
					unsigned int transaction_size = 0;
					while (handled_sectors_count < size) {
						if (lsa > stat->Domain_end_LSA - stat->Domain_start_LSA) {//수정
							lsa = (lsa % (stat->Domain_end_LSA - stat->Domain_start_LSA+1));
						}
						LHA_type internal_lsa = lsa; // 수정
				...
				//Adedd code for debugging. By Seong Jihwan
				if (stat->Stream_id >= 0)
				{
					for (auto itr_test = lpa_set_for_preconditioning.begin(); itr_test != lpa_set_for_preconditioning.end(); itr_test++)
					{
						if (itr_test->first > stat->Domain_end_LSA - stat->Domain_start_LSA)
							std::cout << "### Total_logical_pages_no 를 넘는 lpa가 존재 : " << itr_test->first << std::endl;

					}
				}
				//

				// Added Code for debugging and solving Error . By Seong Jihwan.
				decision_dist_type = stat->Address_distribution_type;
				std::cout << "flow - " << stat->Stream_id << " : distribution_type = " << (int)decision_dist_type << std::endl;
				//
			}//else of if (stat->Type == Utils::Workload_Type::SYNTHETIC)
			std::cout << "precondtion step 2 " << std::endl;
			...
			std::cout << "precondtion step 3 " << std::endl;
			...
			BlockManager->Set_erase_count_for_preconditioning();
			std::cout << "precondtion step 4 " << std::endl;
	...
	{
		// (64bit)0b01를 (lha % page_size_in_sectors)만큼 << 
