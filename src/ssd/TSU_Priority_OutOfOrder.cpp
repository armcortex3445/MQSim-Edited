...
void TSU_Priority_OutOfOrder::Schedule()
{
         ...
         it++)
    {
        ...
        case Transaction_Type::READ:
            switch ((*it)->Source)
            {
            // Transaction_Source_Type에 해당하는 Queue에 TR을 삽입한다. 이때, push_back()함수는 Queue의 meta data정보를 갱신하는 기능도 추가되어있다.
    ...
    //모든 channel에 있는 chip에 Queue를 탐색하여 높은 class의 Queue에 있는 READ/WRITE/ERASE를 처리한다. 
...
//Q,왜 source Queue를 2개까지만 처리하지 3개 처리하면 안되나?
// read TR을 처리하면 1, 처리 안하면 0 반환
bool TSU_Priority_OutOfOrder::service_read_transaction(NVM::FlashMemory::Flash_Chip *chip)
{
    ...
    //FTL mappung과 관련있는 Flash TR은 가장 높은 우선순위를 갖는다.
    if (MappingReadTRQueue[chip->ChannelID][chip->ChipID].size() > 0)
    {
        sourceQueue1 = &MappingReadTRQueue[chip->ChannelID][chip->ChipID];
        //GC가 urgent mode이고, GCReadTRQueue[chip->ChannelID][chip->ChipID]에 TR이 1개 이상이면 if문 실행.
...
//chip에 있는 우선순위가 높은 class의 Queue를 반환
...
//Erase tr을 처리한다면 1 반환, 처리 안하면 0 반환
    ...
    void TSU_Priority_OutOfOrder::estimate_flow_slowdown(NVM_Transaction_Flash* serviced_TR)//FLIN 추가용
    {
