...
namespace SSD_Components
{
	...
	void TSU_Base::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		//In FLIN Algorithm, to estimate slowdown of flow of transaction comple
		if(_my_instance->type == Flash_Scheduling_Type::FLIN)
			if (transaction->Source == Transaction_Source_Type::USERIO|| transaction->Source==Transaction_Source_Type::CACHE)
			{
				_my_instance->estimate_flow_slowdown(transaction);
			}
	...
	//Edited Function for using Die interleavving, SSD internal parallelism. By Seong jihwan.
	bool TSU_Base::issue_command_to_chip(Flash_Transaction_Queue* sourceQueue1, Flash_Transaction_Queue* sourceQueue2, Transaction_Type transactionType, bool suspensionRequired)
	{
		...
		bool is_dispatched = false;//Added variable for using die interleaving.
	...
	// This function uses priority of Transacion Queue, as decribed in section 4.2 in FLIN paper.
	bool TSU_Base::issue_command_to_chip_FLIN(Flash_Transaction_Queue* sourceQueue, Flash_Transaction_Queue* userqueue, unsigned int no_of_priority_classes, bool suspensionRequired, __int8 TRslotExisted)
	{
		flash_die_ID_type dieID;
		flash_page_ID_type pageID; 
		if(TRslotExisted == (1<<0))
		{ 
			for (int i = 0; i<no_of_priority_classes ; i++)
			{
				if (userqueue[i].size())
				{
					dieID = userqueue[i].front()->Address.DieID;
					pageID = userqueue[i].front()->Address.PageID;
					break;
				}
			}
			...
			dieID = sourceQueue->front()->Address.DieID;
			pageID = sourceQueue->front()->Address.PageID;
		}
		...
		if (TRslotExisted & (1 << 0))
		{
			...
			for (unsigned int i = 0; i < die_no_per_chip; i++)
			{
				...
				for (int j = 0; j < no_of_priority_classes; j++) //user
				{
					int k = 0;
				
					for (Flash_Transaction_Queue::iterator it = userqueue[j].begin(); it != userqueue[j].end();)
					{
						if(k==std::pow(2,no_of_priority_classes-j-1))
							break;
						...
						if (transaction_is_ready(*it) && (*it)->Address.DieID == dieID && !(planeVector & 1 << (*it)->Address.PlaneID))
						{
							...
							if (planeVector == 0 || (*it)->Address.PageID == pageID)
							{
								...
								transaction_dispatch_slots.push_back(*it);
								DEBUG(issueCntr++ << ": " << Simulator->Time() << " Issueing Transaction - Type:" << TRTOSTR((*it)) << ", PPA:" << (*it)->PPA << ", LPA:" << (*it)->LPA << ", Channel: " << (*it)->Address.ChannelID << ", Chip: " << (*it)->Address.ChipID);
								update_head_high_TR(it);
								update_transaction_count(*it);
								userqueue[j].remove(it++);
								k++;
								continue;
							}
						}
				...
				if (sourceQueue != NULL && transaction_dispatch_slots.size() < plane_no_per_die)
				{
					for (Flash_Transaction_Queue::iterator it = sourceQueue->begin(); it != sourceQueue->end();)
					{	
						if (transaction_is_ready(*it) && (*it)->Address.DieID == dieID && !(planeVector & 1 << (*it)->Address.PlaneID))
						{
							//Check for identical pages when running multiplane command
			...
			for (unsigned int i = 0; i < die_no_per_chip; i++)
			{
				...
				for (int j = 0; j < no_of_priority_classes; j++)
				{
					
					if (userqueue[j].size()>0 && transaction_dispatch_slots.size() < plane_no_per_die)
					{
						int k = 0;
						for (Flash_Transaction_Queue::iterator it = userqueue[j].begin(); it != userqueue[j].end();)
						{	
	...
	// Added Function for using CIF Algorithm. By Soeng Jihwan.
	// This function is issue_command_to_chip_FLIN() , just function name is differnet.
	bool TSU_Base::issue_command_to_chip_CIF(Flash_Transaction_Queue* sourceQueue, Flash_Transaction_Queue* userqueue, unsigned int no_of_priority_classes, bool suspensionRequired, __int8 TRslotExisted)
	{
		flash_die_ID_type dieID;
		flash_page_ID_type pageID;
		if (TRslotExisted == (1 << 0))
