...
#define All_VALID_PAGE 0x0000000000000000ULL
	...
	*/
	...
	public:
		...
		int Ongoing_gc_program_count;
		void Erase();
		uint64_t ongoing_gc_wl_count = 0; // Added variable for debugging. by Seong Jihwan.
	};
	...
	public:
		...
		virtual void Set_erase_count_for_preconditioning() = 0; //Added Function for precondition. This function set erace count of block 0~3 in precondition. By Seong Jihwan.
		...
		void GC_program_transaction_serviced(const NVM::FlashMemory::Physical_Page_Address& page_address);// Added Function to solve race condition, when target block of GC write transacion is GC candidate. By Seong Jihwan. 
		bool Is_gc_program_finished(const NVM::FlashMemory::Physical_Page_Address& block_address);// Added Function to solve race condition, when target block of GC write transacion is GC candidate. By Seong Jihwan. 
	protected:
		...
		void gc_program_transaction_issued(const NVM::FlashMemory::Physical_Page_Address& page_address); // Added Function to solve race condition, when target block of GC write transacion is GC candidate. By Seong Jihwan. 
