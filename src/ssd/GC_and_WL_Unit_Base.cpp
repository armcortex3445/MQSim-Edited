...
namespace SSD_Components
{
	...
	void GC_and_WL_Unit_Base::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction)
	{
		...
		switch (transaction->Source) {
			...
			case Transaction_Source_Type::CACHE:
				...
				if (_my_instance->block_manager->Block_has_ongoing_gc_wl(transaction->Address)) {
					if (_my_instance->block_manager->Can_execute_gc_wl(transaction->Address) && _my_instance->block_manager->Is_gc_program_finished(transaction->Address)) {
						...
						if (block->Current_page_write_index >= block->Invalid_page_count) {
							...
							bool gc_page_locked_new = false; //내가 추가함.
							for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
								if (_my_instance->block_manager->Is_page_valid(block, pageID)) {
									...
									if (!_my_instance->address_mapping_unit->is_logical_page_locked_for_gc(block->Stream_id, lpa, block->Holds_mapping_data))
									{
										gc_page_locked_new = true;
										//std::cout << "GC locked page that was not locked untill now. (stream id, lpa, mapping data ) =  "<<block->Stream_id<<" , "<<lpa<<" , "<<block->Holds_mapping_data<< std::endl;
										//std::cout << "new GC locekd page addr =  " << gc_wl_candidate_address.ChannelID << " , " << gc_wl_candidate_address.ChipID << " , " << gc_wl_candidate_address.DieID << " , " << gc_wl_candidate_address.PlaneID << " , " << gc_wl_candidate_address.BlockID << " , " << gc_wl_candidate_address.PageID << std::endl;
										if (block->Holds_mapping_data)
										{
											_my_instance->address_mapping_unit->Set_barrier_for_accessing_mvpn(block->Stream_id, lpa);
										}
										else
										{
											_my_instance->address_mapping_unit->Set_barrier_for_accessing_lpa(block->Stream_id, lpa);
										}
									}
							...
							/*if (gc_page_locked_new == true)
							{
								std::cout << "Current_page_write_index , Invalid_page_count = " << block->Current_page_write_index << " , " << block->Invalid_page_count << std::endl;
								std::cout << "End new GC locekd block addr =  " << gc_wl_candidate_address.ChannelID << " , " << gc_wl_candidate_address.ChipID << " , " << gc_wl_candidate_address.DieID << " , " << gc_wl_candidate_address.PlaneID << " , " << gc_wl_candidate_address.BlockID <<" Block GC_wl going count ="<<block->ongoing_gc_wl_count<< std::endl;
								
							}*/
						}
						else {
							PRINT_ERROR("In handle_transaction_serviced_signal_from_PHY(). Current_page_write_index is lower than Invalid_page_count  ")
						}
		...
		switch (transaction->Type) {
			case Transaction_Type::READ:
			{
				...
				if (pbke->Blocks[transaction->Address.BlockID].Holds_mapping_data) {
					...
					if (mppa == transaction->PPA) {
						...
						if (transaction->PPA ==512)
						{
							std::cout << "read GC transaction  (Stream id , MVPN) = "<<transaction->Stream_id<<" ,"<<transaction->LPA << std::endl;
						}
						...
						//Added Code for Debugging. By Seong jihwan
						std::cout << "Read Transaction has  mapping information." << std::endl;
						std::cout << "(TR->stream_id, TR->lpa,TR->ppa, GTD[lpa].mppa,GTD[lpa].timestamp) =  " << transaction->Stream_id << " , " << transaction->LPA << " , " << transaction->PPA
							<< " , " << mppa << " , " << page_status_bitmap << std::endl;
						//
					...
					if (ppa == transaction->PPA) {
						...
						//Added Code for Debugging. By Seong jihwan
						std::cout << "Read Transaction은 Data를 갖고 있다." << std::endl;
						std::cout << "(TR->stream_id, TR->lpa,TR->ppa, GMT[lpa].ppa,GMT[lpa].written_stata_bitmap) =  " << transaction->Stream_id << " , " << transaction->LPA << " , " << transaction->PPA
							<< " , " << ppa << " , " << page_status_bitmap << std::endl;
						//
						PRINT_ERROR("Inconsistency found when moving a page for GC/WL!")
					}
				}
			...
			case Transaction_Type::WRITE:
				_my_instance->block_manager->GC_program_transaction_serviced(transaction->Address);
	...
	void GC_and_WL_Unit_Base::run_static_wearleveling(const NVM::FlashMemory::Physical_Page_Address plane_address)
	{
		...
		NVM::FlashMemory::Physical_Page_Address gc_candidate_address(wl_candidate_address);
		...
		if (block_manager->Can_execute_gc_wl(wl_candidate_address)&&block_manager->Is_gc_program_finished(wl_candidate_address)) {//If there are ongoing requests targeting the candidate block, the gc execution should be postponed
			...
			if (block->Current_page_write_index >= block->Invalid_page_count) {//If there are some valid pages in block, then prepare flash transactions for page movement
				...
				for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
					if (block_manager->Is_page_valid(block, pageID)) {
						...
						//내가 추가
						PPA_type ppa = _my_instance->address_mapping_unit->Convert_address_to_ppa(wl_candidate_address);
						if (ppa == 512 )
						{
							LPA_type lpa = _my_instance->flash_controller->Get_metadata(wl_candidate_address.ChannelID, wl_candidate_address.ChipID, wl_candidate_address.DieID, wl_candidate_address.PlaneID, wl_candidate_address.BlockID, wl_candidate_address.PageID);
							std::cout << "run_static_wearleveling() cause  GC transaction.  stream id ="<< pbke->Blocks[wl_candidate_address.BlockID].Stream_id <<", LPA = " << lpa << std::endl;
						}
				...
				std::cout << "Current_page_write_index , Invalid_page_count = " << block->Current_page_write_index << " , " << block->Invalid_page_count << std::endl;
				PRINT_ERROR("In run static wearleveling(). Current_page_write_index is lower than Invalid_page_count  ")
			}
	...
	//Added Function for solving race condition. By Seong Jihwan.
	bool GC_and_WL_Unit_Base::is_going_write_gc_wl_candidate(const NVM::FlashMemory::Physical_Page_Address& plane_address, const flash_block_ID_type gc_wl_candidate_block_id)
	{
		PlaneBookKeepingType* pbke = block_manager->Get_plane_bookkeeping_entry(plane_address);
		bool ongoing_write = false;
		if (pbke->Blocks[gc_wl_candidate_block_id].Current_page_write_index != pages_no_per_block)
		{
			ongoing_write = true;
			return ongoing_write;

		}
		NVM::FlashMemory::Physical_Page_Address addr(plane_address);
		for (flash_page_ID_type pageID = pbke->Blocks[gc_wl_candidate_block_id].Current_page_write_index; pageID >= 0; pageID--) {
			if (block_manager->Is_page_valid(&pbke->Blocks[gc_wl_candidate_block_id], pageID)) {
				addr.PageID = pageID;
				LPA_type lpa = flash_controller->Get_metadata(addr.ChannelID, addr.ChipID, addr.DieID, addr.PlaneID, addr.BlockID, addr.PageID);
				if (lpa == NO_LPA)
				{
					bool ongoing_write = true;
					break;
				}
			}
		}
