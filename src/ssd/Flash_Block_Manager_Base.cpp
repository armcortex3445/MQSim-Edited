...
namespace SSD_Components
{
	...
	Flash_Block_Manager_Base::Flash_Block_Manager_Base(GC_and_WL_Unit_Base* gc_and_wl_unit, unsigned int max_allowed_block_erase_count, unsigned int total_concurrent_streams_no,
		...
		block_no_per_plane(block_no_per_plane), pages_no_per_block(page_no_per_block),random_generator(die_no_per_chip*max_allowed_block_erase_count)
	{
				...
				for (unsigned int dieID = 0; dieID < die_no_per_chip; dieID++) {
					...
					for (unsigned int planeID = 0; planeID < plane_no_per_die; planeID++) {
						...
						for (unsigned int blockID = 0; blockID < block_no_per_plane; blockID++) {
							...
							//추가
							plane_manager[channelID][chipID][dieID][planeID].Blocks[blockID].Ongoing_gc_program_count = 0;
	...
	Block_Pool_Slot_Type* PlaneBookKeepingType::Get_a_free_block(stream_id_type stream_id, bool for_mapping_data)
	{
		...
		new_block->Holds_mapping_data = for_mapping_data; //data block인지  address translation block인지 구분 
	...
	void Flash_Block_Manager_Base::GC_WL_started(const NVM::FlashMemory::Physical_Page_Address& block_address)
	{
		...
		plane_record->Blocks[block_address.BlockID].ongoing_gc_wl_count++;
	}
	// Added Function to solve race condition, when target block of GC write transacion is GC candidate. By Seong Jihwan. 
	void Flash_Block_Manager_Base::gc_program_transaction_issued(const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType* plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Blocks[page_address.BlockID].Ongoing_gc_program_count++;
	}
	// Added Function to solve race condition, when target block of GC write transacion is GC candidate. By Seong Jihwan. 
	void Flash_Block_Manager_Base::GC_program_transaction_serviced(const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType* plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		plane_record->Blocks[page_address.BlockID].Ongoing_gc_program_count--;
	}
	// Added Function to solve race condition, when target block of GC write transacion is GC candidate. By Seong Jihwan. 
	bool Flash_Block_Manager_Base::Is_gc_program_finished(const NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		PlaneBookKeepingType* plane_record = &plane_manager[page_address.ChannelID][page_address.ChipID][page_address.DieID][page_address.PlaneID];
		return plane_record->Blocks[page_address.BlockID].Ongoing_gc_program_count==0;
	}
	...
	// Edited Function becase of wrong code. By Seong jihwan.
	bool Flash_Block_Manager_Base::Is_page_valid(Block_Pool_Slot_Type* block, flash_page_ID_type page_id)
	{  // modification : (uint64_t)1) << page_id  => (uint64_t)1) << page_id % 64 
