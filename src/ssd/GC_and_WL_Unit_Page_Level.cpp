...
namespace SSD_Components
{
	...
	void GC_and_WL_Unit_Page_Level::Check_gc_required(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		if (free_block_pool_size < block_pool_gc_threshold) {
			...
			if (gc_candidate_address.ChannelID == 0 && gc_candidate_address.ChipID == 0 && gc_candidate_address.DieID == 0 && gc_candidate_address.PlaneID == 0 && gc_candidate_address.BlockID == 0)
			{
				std::cout << "Check_gc_required () . address (0,0,0,0,0) is gc candidate" << std::endl;
				for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
					if (block_manager->Is_page_valid(block, pageID)) {

						LPA_type lpa = flash_controller->Get_metadata(gc_candidate_address.ChannelID, gc_candidate_address.ChipID, gc_candidate_address.DieID, gc_candidate_address.PlaneID, gc_candidate_address.BlockID, gc_candidate_address.PageID);
						std::cout << " address(0,0,0,0,0) valid page.  (stream id, lpa, page id) = " << block->Stream_id << ", " << lpa << ", " << pageID << std::endl;
					}
			...
			if (block_manager->Can_execute_gc_wl(gc_candidate_address) && block_manager->Is_gc_program_finished(gc_candidate_address)) {
				...
				if (block->Current_page_write_index >= block->Invalid_page_count ) {
					...
					PRINT_ERROR("In Check_gc_required(). Current_page_write_index is lower than Invalid_page_count  ")
				}
